...
const ContentList:React.FC<ContentListProps> = ({movieList, movie, tv, trending}) => {
    ...
    const movieDetails = async (modalInfoData:any)=>{
        ...
        try{
            ...
            let media_type = modalInfoData.media_type !== undefined ? modalInfoData.media_type: movie !==undefined ? movie: tv;
    ...
    return (
        <div className='contentListWrapper'>
            ...
            {movieList.map((ele, index) => {
                let trimmedTitle = (ele?.title && ele?.title.slice(0, 30)) + ((ele?.title &&ele.title.length > 30 )? "..." : "");
                let trimmedName = (ele?.name && ele?.name.slice(0, 30)) + ((ele?.name && ele?.name.length > 30) ? "..." : "");
                return (
                    <div className='individualBlockElem' key={index} 
                    onClick={() => openmoreInfoModal(index)}
                    >
                        <div className='topSection'>
                            <img width={284} height={200} src={`${img_300}${ele.poster_path !== null? ele.poster_path: ele.backdrop_path}`} alt={trimmedTitle || trimmedName} />
                        </div>
                        <div className='rating'>{Math.round(ele.vote_average * 10) / 10} </div>
                        <div className='bottomSection'>
                            <h3>
                                {(tv === 'tv' || ele.media_type==='tv') ? (trimmedName) : (trimmedTitle)}
                            </h3>
                            <div className='movieInfo'>
                                {/* <p>{capitalizeFirstLetter(ele.media_type)} </p> */}
                                <p> {(tv === 'tv' || ele.media_type==='tv') ? ele.first_air_date : ele.release_date} </p>
                            </div>
                        </div>
